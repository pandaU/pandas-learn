...
if __name__ == '__main__':
    ...
    print("original mean: %s" % np.mean(features, axis=0))
    # print("std: %s" % np.std(features, axis=0))
    # print("origin data")
    # features_new = preprocessing.StandardScaler().fit_transform(features)
    # #print(features_new)
    # print("new mean: %s" %np.mean(features_new, axis=0))
    # print("new std: %s" %np.std(features_new, axis=0))
    ...
    # null_data = vstack((features, array([nan, nan, nan, nan])))
